// ANEStoriesCPUOps.h — CPU operations for iOS ANE training
...
static void rmsnorm_bwd(float *dx, float *dw, const float *dy, const float *x, const float *w, int d, int S) {
    ...
    for (int i=0; i<d; i++) {
        ...
        vDSP_vmul(x+i*S, 1, dot, 1, rms_tmp, 1, (vDSP_Length)S);       // tmp = x[i] * dot
        vDSP_vsub(rms_tmp, 1, dx+i*S, 1, dx+i*S, 1, (vDSP_Length)S);   // dx = w[i]*dy[i] - x[i]*dot
